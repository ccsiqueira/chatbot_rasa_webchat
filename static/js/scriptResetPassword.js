...
function setUserResponse(message) {
    var UserResponse = '<img class="chatbot userAvatar" src=' + "./static/img/userAvatar.jpg" + '><p class="chatbot userMsg">' + message + ' </p><div class="chatbot clearfix"></div>';
        ...
        if (response.length < 1) {
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("image")) {
                    var BotResponse = '<div class="chatbot singleCard">' + '<img class="chatbot imgcard" src="' + response[i].image + '">' + '</div><div class="chatbot clearfix">';
                    $(BotResponse).appendTo(".chatbot.chats").hide().fadeIn(1000);
                }
...
function createChart(title, labels, backgroundColor, chartsData, chartType, displayLegend) {
    ...
    var html = '<div class="chatbot chart-container"> <span class="chatbot modal-trigger" id="expand" title="expand" href="#modal1"><i class="chatbot fa fa-external-link" aria-hidden="true"></i></span> <canvas id="chat-chart" ></canvas> </div> <div class="chatbot clearfix"></div>'
    $(html).appendTo('.chats');
